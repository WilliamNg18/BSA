...
import { BASELINE_DEFAULTS, baselineDefaultCopy } from "../../src/lib/domain/baseline-defaults";
...
vi.mock("../../src/lib/domain/baseline-defaults", async (importOriginal) => {
  const original = await importOriginal<typeof import("../../src/lib/domain/baseline-defaults")>();
  return { ...original, BASELINE_DEFAULTS: { ...original.BASELINE_DEFAULTS, volume: 12_345, gatheringMinutes: 7.5, judgingMinutes: 3.25 } };
});
...
describe("calculator UI consumes canonical defaults", () => {
  it.each([false, true])("generates labels on the %s register surface from changed defaults", (register) => {
    ...
    expect(html).toContain("Gathering 7.5 minutes and judging 3.25 minutes");
    ...
    expect(html).not.toContain("Gathering 5 minutes and judging 2 minutes");
  });
});