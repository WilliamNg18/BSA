...
it("keeps the actual approved referral and RB code before supporting metadata", () => {
  ...
  expect(html.slice(response, metadata)).toContain(draft.rbCode);
  expect(html.slice(response, metadata)).toContain(draft.note);
  expect(before.records.at(-1)?.approvedDraft).toBeDefined();
  expect(draft.note).not.toContain(caseById(id)!.pharmacySupplyRecord!.brandManufacturer);
