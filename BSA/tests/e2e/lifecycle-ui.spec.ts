...
test("Task9 C shows both conflict values, approved note and confirmation without resolving evidence", async ({ page }) => {
  ...
  await page.getByRole("banner").getByRole("switch").setChecked(true);
  await operatorDecision(page).getByRole("button", { name: "Apply suggestion", exact: true }).click();
  await performDecision(page, "REQUEST_INFORMATION", { openAudit: true });
  await page.getByRole("link", { name: "View pharmacy claim", exact: true }).click();
  ...
  await page.getByRole("textbox", { name: "Confirm", exact: true }).fill("Please review both values against the synthetic form");
  await page.getByRole("button", { name: "Send confirmation", exact: true }).click();
  await expect(detail(page)).toContainText("Resubmitted, awaiting re-check");
...
test("Task19 D blocks Type 2 before capture and E clears by code without entering the queue", async ({ page }) => {
  ...
  await expect(operatorDecision(page).getByRole("radiogroup", { name: "Decision", exact: true })).toHaveCount(0);
  await expect(page.getByRole("radio", { name: "Refer back", exact: true })).toHaveCount(0);
  ...
  await page.getByRole("link", { name: "View NHSBSA case", exact: true }).click();
  await expect(history(page).getByRole("status")).toHaveText(LIFECYCLE_LABELS.released_to_pricing.nhsbsa.on);
  await expect(operatorDecision(page).getByRole("radiogroup", { name: "Decision", exact: true })).toHaveCount(0);
