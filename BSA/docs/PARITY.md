...
| 31 | Actual hosted root/deep-link evidence | Issue #48 selects existing App Service F1 at `/` | Existing URL/resource and OIDC setup confirmed by coordinator. Portable strict headers and exact clean build commit still require hosted verification; old HTTP 200 without CSP is not acceptance. F1 has no slots. Local CI cannot establish hosted behaviour | Release not yet verified | Coordinator #48; D records actual release evidence |
