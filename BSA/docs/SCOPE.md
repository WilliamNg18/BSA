...
**Snapshot:** main `22ec3459dee77818146ce7dbb99ca5baf47bb9ea`, 12 September
2026, includes S PR #22. Other branch work below remains unmerged. Baseline
evidence still names its original revision rather than being relabelled as a
fresh run. This latest request has **eight chapters**, not the currently
implemented seven chapters/eight stops.
...
* **T prepared:** PR #30 at `02aa77a` reports check and 555 unit tests passed,
  including the updated `tour.test.ts` chapter contracts. Eight chapters/nine
  stops and the cycle guide are implemented on that branch. Six focused
  browsers remain pending; no new chapter screenshot or runtime PASS is claimed.
...
| **A3 Separate six-stage pipeline chapter:** phase-resolving pain | **In progress · T #26 / PR #30**; current pipeline on main | Main combines pipeline and cards. T `02aa77a` separates pipeline to `/#pipeline` while retaining six stages and bounded phase-resolving markers; it is not merged or browser-accepted yet. | **V** `overview-pipeline` shows baseline combined content. **T prepared** supplies updated unit contracts; `pipeline.spec.ts`: `shared two-second clock sequences kernel phases, resolves built pain only, and cancels safely` is **R pending**. T's independent reachability/runtime proof pending. |
| **A4 Four cases chapter:** Off pain, On result, both actions | **In progress · T #26 / PR #30** | T `02aa77a` moves A-D cards into the separately reachable `/#cases` chapter with Open case and Follow actions. This increment is missing on main until the branch merges and is accepted. | **U/P** `agent.test.ts` pins canonical outcomes; **V** baseline combined A-D view. **T prepared** units pass; six focused browsers including order/actions remain pending, as does R's wider regression run. |
...
| **Rail:** Next/Back across routes, dismiss/restore | **In progress · T #26; S fixes merged `22ec3459`** | Existing rail crosses pharmacy/queue/claims. S's AppShell dismiss/restore focus and Reset computed-motion corrections are merged. Eight-chapter mapping and integrated regressions remain T/R-owned. | **S merged** proves its named affected focus/motion slice. **U/P** guarded Alt shortcuts; `tour.spec.ts` forward/back/dismiss/restore tests remain part of the pending integrated T/R acceptance. |
...
| --- | --- | --- |
| T | `williamng18-eight-chapter-tour-and-referral-cycle`, #26 / PR #30 | `02aa77a`: eight chapters, pipeline/Four cases split, two-places contrast and whole-cycle claims guide; check/555 units pass, six browsers pending |
