...
const canonical = PLAYABLE_CASES.map((item) => ({
  id: item.id,
...
const rows = [...canonical, ...BACKGROUND_CASES.map((c) => ({ id: c.id, state: c.initialState })), ...QUEUE_FILLER];
...
// Only playable advice contributes assembly timing; background is never a model run.
...
export const BASELINE_PROVENANCE = {
  ...
  fillerCount: QUEUE_FILLER.length + BACKGROUND_CASES.length,
