...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  { name: "read_image_region", purpose: "Read the located endorsement margin and item line of the scanned form", input: "{ imageRef, regionId }", output: "{ text, confidence, bbox }", mock: "Pre-annotated regions on synthetic forms", production: "Document layout analysis over NHSBSA's existing image store", cls: "existing" },
  ...
  { name: "run_endorsement_checks", purpose: "Deterministic requirement and mandatory-field checks", input: "{ clause, facts, extracted }", output: "{ requirementResults[], mandatory[] }", mock: "Same code as production", production: "Serverless deterministic functions with unit tests", cls: "deterministic" },
  { name: "validate_citation", purpose: "Confirm the cited span exists in the corpus for the version in force", input: "{ clauseId, version, quotedSpan }", output: "{ valid }", mock: "Same code as production", production: "Serverless deterministic functions", cls: "deterministic" },
  { name: "write_decision_record", purpose: "Append the case pack and, later, the human decision", input: "{ casePack | decision }", output: "{ recordId }", mock: "In-memory append-only array", production: "Append-only record store", cls: "deterministic" },
];
...
export const AGENT_VERSION = "prototype-0.6 (interpretation step mocked; production: constrained model call)";
