...
export function PharmacyDraftFields({ draft, original, channel, update, correction = false }: {
  draft: PharmacyCorrectionDraft; original: PharmacyCorrectionDraft; channel: ItemChannel;
  update: (draft: PharmacyCorrectionDraft) => void; correction?: boolean;
}) {
  const id = useId();
  const eps = draft.epsPrescription;
  const paper = draft.paperDeclaration;
  const declaration = draft.declaration;
  const highlight = (before: unknown, after: unknown) => draft.appliedSuggestion && before !== after ? "border-primary ring-2 ring-primary" : "";
  ...
  return <fieldset className="space-y-3" data-pharmacy-draft>
    ...
    {channel === "paper" && <p id={`${id}-origin`} className="text-xs">declared by the pharmacy, not read from the form</p>}
    ...
    <div className="space-y-1">
      ...
      <Input id={correction ? "claim-endorsement" : "endorsement"} value={draft.endorsementText} className={highlight(original.endorsementText, draft.endorsementText)}
    ...
    {eps?.items[0]?.dispensedCode === EPS_SUPPLY_RULE.productCode && <div className="grid grid-cols-2 gap-3">
      {(["brandManufacturer", "packSize", "form"] as const).map((field) => <label className={`grid gap-1 ${field === "brandManufacturer" ? "col-span-2" : ""}`} key={field}>
        ...
        <Input id={field === "brandManufacturer" ? "eps-manufacturer" : field === "packSize" ? "eps-pack" : "eps-form"}
          ...
          value={eps.supplyEvidence?.[field] ?? ""} className={highlight(original.epsPrescription?.supplyEvidence?.[field], eps.supplyEvidence?.[field])}
