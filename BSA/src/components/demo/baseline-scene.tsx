...
export function BaselineScene() {
  ...
  return <section aria-label="Shared scenario estimates" className="space-y-3 rounded-xl border bg-card p-5" data-scene-estimates>
    ...
    {result ? <dl className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div><dt>{enabled ? "With agent" : "Today"}: items per operator a month</dt><dd className="break-all" data-scene-capacity><SceneEstimateNumber value={enabled ? result.capacity.withAgent : result.capacity.today} enabled={enabled} scenario={result} /></dd></div>
